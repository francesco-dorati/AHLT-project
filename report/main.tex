...
\textbf{macro-averaged F1 = 68.2\%}. The large gap between the two (18.4~pp)
...
\hline
brand   &  92.9\% & 89.1\% & \cellcolor{red!12}$-$3.8 \\
drug    &  92.1\% & 92.0\% & $-$0.1 \\
\rowcolor{green!18} group   &  73.3\% & 79.8\% & \textbf{+6.5} \\
\rowcolor{green!18} drug\_n &  12.6\% & 15.6\% & \textbf{+3.0} \\
\hline
\textbf{M.avg} & \textbf{67.7\%} & \textbf{69.1\%} & \cellcolor{green!18}\textbf{+1.4} \\
\hline
\end{tabular}
...
\begin{figure}[H]
    ...
    \caption{Campaign progression from Phase~B (smoke-test, 37.6\%
    devel m-F1) to Phase~I (Qwen×r=32 on test, 58.4\%). The two
    dotted lines show the best few-shot (39.2\%) and best fine-tune
    (63.8\%) devel anchors. The fine-tune jumps (Phase~D and~G) are
...
zero-shot (2.2\%, the model ignores the tagging instruction without
a demonstration) to five shots (37.6\%), then plateaus. Fifteen
shots wins by a small margin (39.2\%) but at the cost of a long-tail
...
\begin{figure}[H]
    ...
    $\sim$8~pp above micro-F1, meaning the model is better at
...
\texttt{prompts01} on micro-F1 (prompts03/15 hits \textbf{40.9\%},
the best few-shot micro in the campaign) \emph{but collapse macro-F1}
from 32.8 down to 26.6\,\%: the terse decision-guide
...
\begin{figure}[H]
    ...
    (prompts03/15 = 40.9\%) has the \emph{worst} macro (26.6\%)
    because \textit{drug\_n} recall goes to zero. For a
    rare-class-sensitive task, \texttt{prompts01/15} (macro 32.8\%)
...
the single most informative cell in the entire campaign:
\texttt{llama-p01-15} and \texttt{llama-p03-15} (the two best-micro
configurations) both have \textit{drug\_n}~F1~=~0. The model, under
prompt-only supervision, is \emph{unable} to recover even one
correct \textit{drug\_n} span. A class-balanced sampler (last bar)
lifts \textit{drug\_n}~F1 from 0 to 25.3\,\% but costs~6\,pp of
...
\begin{figure}[H]
    ...
    from 5--15 demonstrations; \textit{drug\_n} is not. Only the
    class-balanced sampler (last group) lifts \textit{drug\_n} above
    zero, at a 6\,pp cost on micro-F1.}
    \label{fig:13-pc-fs}
\end{figure}
...
\hline
414918 & p01 & 0 & --- & \cellcolor{red!15}2.2 & \cellcolor{red!15}1.9 & \cellcolor{red!15}2.9 \\
414919 & p01 & 3 & --- & 29.5 & 29.3 & 42.5 \\
414791 & p01 & 5 & --- & 37.6 & 34.5 & 47.1 \\
414920 & p01 & 10 & --- & 35.3 & 34.3 & 46.1 \\
414921 & p01 & 15 & --- & 39.2 & 32.8 & 48.8 \\
414922 & p02 & 5 & --- & 38.2 & 28.9 & 46.8 \\
414923 & p03 & 5 & --- & 39.0 & 25.6 & 49.1 \\
414924 & p01 (Qwen) & 5 & --- & 35.3 & 22.0 & 45.3 \\
415438 & p02 & 15 & --- & 39.6 & 28.7 & 48.6 \\
415439 & p03 & 15 & --- & \cellcolor{green!18}\textbf{40.9} & 26.6 & \cellcolor{green!18}\textbf{51.0} \\
415466 & p01 & 15 & yes & 33.2 & \cellcolor{green!18}\textbf{32.5} & 41.1 \\
\hline
\end{tabular}
\caption{Few-shot devel results (\textit{Llama 3.2 3B Instruct} +
4-bit quant unless labelled \textit{Qwen}). All F1 in \%.
\textbf{Best micro: p03/15 = 40.9\%}. \textbf{Best macro: balanced
sampler = 32.5\%}. The two optima disagree because prompts03 zeros
out \textit{drug\_n}.}
\label{tab:13-fewshot}
\end{table}

Qwen~2.5~3B at 5-shot devel (35.3\%) \emph{underperforms} Llama
(37.6\%) by 2.3\,pp. Its \textit{group} recall collapses to 4.0\%,
...
resulting system achieves \textbf{devel m-F1 = 59.7\%}, a
\textbf{+20.5\,pp} jump over the best few-shot system (39.2\%) at
...
\hline
 & P & R & F1 \\
\hline
brand   & 74.9\% & 72.5\% & \cellcolor{green!15}73.7\% \\
drug    & 67.6\% & 59.1\% & 63.0\% \\
drug\_n & 10.5\% & 24.0\% & \cellcolor{red!15}14.6\% \\
group   & 54.0\% & 55.1\% & 54.5\% \\
\hline
\end{tabular}
\end{center}
\textit{Every} class improves versus the matched few-shot system
(15-shot prompts01): \textit{brand} $60.2\to73.7$, \textit{drug}
$41.3\to63.0$, \textit{group} $29.3\to54.5$, \textit{drug\_n}
$0.0\to14.6$. Macro-F1 nearly doubles ($32.8\to51.5$).
...
every metric except micro-F1-by-a-hair}: devel~m-F1 $61.2$
(vs~$59.7$, $+1.5$\,pp), devel~M-F1 $56.6$ (vs~$51.5$, $+5.1$\,pp),
and \textit{drug\_n}~F1 $33.5$ (vs~$14.6$, $+18.9$\,pp). The
macro-F1 gap is almost entirely the \textit{drug\_n} column: Qwen is
...
precision across the board (\textit{brand}~P $84.2$ vs Llama's
$74.9$). \emph{The lesson: zero- and few-shot rankings of base models
...
\begin{figure}[H]
    ...
    micro-F1 by \textbf{+4.1\,pp} and macro-F1 by \textbf{+8.1\,pp}
    over the Phase-D baseline. Adding five more training epochs (15
    vs 10) lifts micro-F1 by only +1.8\,pp. Rank capacity matters
...
baseline) reaches \textbf{devel m-F1 = 63.8\,\%} and \textbf{M-F1 =
59.6\,\%}. Every class improves: \textit{drug\_n}~$+22.7$,
\textit{group}~$+5.5$, \textit{brand}~$+3.0$, \textit{drug}~$+1.4$.
...
m-F1 $=61.5\,\%$, $+1.8$\,pp over Phase-D but still $-2.3$\,pp behind
...
the 15-shot \texttt{prompts01} setup (the best-micro few-shot config).
A balanced sampler guarantees $\geq3$ demonstrations per class,
\emph{including} \textit{drug\_n}. The effect is symmetric: micro-F1
drops 6\,pp (from 39.2 to 33.2\,\%, because we gave up $\sim$3
common-class demos to make room for rare-class ones), macro-F1 is
essentially flat (32.8 vs~32.5), and \textit{drug\_n}~F1 goes from 0
to 25.3\,\%. \emph{Few-shot cannot simultaneously cover the head and
the tail of the class distribution with only~15 demonstrations}; but
fine-tuning with $r=32$ solves this directly on the
\textit{drug\_n} column (37.3\,\%).
...
\begin{figure}[H]
    ...
    The balanced sampler lifts \textit{drug\_n} by +25.3\,pp but
    loses ground on all three common classes.}
    \label{fig:13-bal}
\end{figure}
...
Devel m-F1 $=63.0\,\%$ (0.8\,pp \emph{below} Llama~r=32) but devel
\textbf{M-F1 = 60.8\,\%} (best macro of the campaign) and
\textbf{\textit{drug\_n} F1 = 44.7\,\%} (best rare-class of the
campaign). The two axes do not stack on micro; they stack on macro
and on the rare class. Figure~\ref{fig:13-heat} gives the full
...
58.9\,\%} (vs 57.0\% for $r=8$ --- a +1.9\,pp test-set gain from a
training-time ablation). The devel$\to$test gap is 4.9\,pp (vs 2.7\,pp
for $r=8$), so the larger adapter \emph{does} overfit devel slightly
more than $r=8$, but not enough to erase the gain. \textbf{Test-set
\textit{drug\_n} F1 jumps $27.9 \to 41.7\,\%$} (+13.8\,pp), the single
biggest per-class test-set gain of the campaign.
...
drug\_n F1 & \textbf{41.7\,\%} & 35.8\,\% & $-$5.9\,pp \\
brand F1 & 68.9\,\% & \textbf{75.0\,\%} & $+$6.1\,pp (Qwen wins) \\
\hline
\end{tabular}
\end{center}
...
\textbf{Qwen overfit \textit{drug\_n} on devel} ($44.7\to35.8$,
$-8.9$\,pp), while Llama's \textit{drug\_n} prediction \emph{gained}
on the unseen split ($37.3\to41.7$, $+4.4$\,pp). Llama~$r=32$ is
therefore the single best configuration on \emph{both} devel
\emph{and} test, \emph{both} micro \emph{and} macro.
...
\begin{figure}[H]
    ...
    two test-run fine-tune configurations. The Qwen overfit on
    devel (44.7\% devel $\to$ 35.8\% test) is the single biggest
    surprise of Phase~I; Llama holds (and slightly improves) under
    the same split change.}
    \label{fig:13-drugn}
\end{figure}
...
\hline
414926+415067   & llama32B3 & 8  & 10 & 59.7 & 51.5 & 62.1 \\
415440+415467   & qwen25B3  & 8  & 10 & 61.2 & 56.6 & 63.7 \\
415465+415516   & llama32B3 & 8  & 15 & 61.5 & 56.8 & 64.0 \\
\rowcolor{green!18}\textbf{415464+415481} & \textbf{llama32B3} & \textbf{32} & \textbf{10} & \textbf{63.8} & 59.6 & \textbf{66.3} \\
415524+415683   & qwen25B3  & 32 & 10 & 63.0 & \cellcolor{green!18}\textbf{60.8} & 64.9 \\
\hline
\end{tabular}
...
\begin{figure}[H]
    ...
    \caption{Headline devel F1 across all nine flagship
    configurations, from zero-shot (2.2\%) to the best fine-tune
    (63.8\%). Lighter bars are macro-F1 shadows. The pink highlight
...
Few-shot is free (0 GPU-hours, 39--40\% micro). Fine-tuning costs
5--8~GPU-hours and buys a $\sim$24\,pp absolute improvement. Among
fine-tune configurations, \textbf{$r=32$ is the cheapest large-gain
move}: +4.1\,pp over the Phase-D baseline for only +12\,min of
training time. 15~epochs costs +2\,h~50\,m and buys less (+1.8\,pp).

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\linewidth]{fig13_cost_scatter.pdf}
    \caption{GPU-hours vs devel micro-F1. Few-shot is free but
    capped around 40\%. Fine-tuning at $r=32$ (stars) is the
    cheapest way to reach the 63\% regime; 15 epochs at $r=8$ is
...
\hline
415437 & Few-shot best (p01/15) & 39.2 & \cellcolor{red!12}36.2 & \cellcolor{red!12}23.0 & \cellcolor{red!12}45.9 \\
415436 & FT Llama $r=8$ / 10ep  & 59.7 & 57.0 & 51.5 & 60.9 \\
\rowcolor{green!18}\textbf{415523} & \textbf{FT Llama $r=32$ / 10ep} & \textbf{63.8} & \textbf{58.9} & \textbf{55.7} & \textbf{62.5} \\
415762 & FT Qwen $r=32$ / 10ep  & 63.0 & 58.4 & 55.1 & 61.7 \\
\hline
\end{tabular}
\caption{Final test-set evaluation (\texttt{prompts01}, 4-bit quant).
\textbf{Best system: Llama + $r=32$}. It wins on every test metric
(micro, macro, span-F1), reverses the devel-set macro order (Qwen was
winning devel macro by 1.2\,pp), and is within 3.1\,pp of
few-shot's devel--test generalisation gap.}
\label{tab:13-test}
\end{table}
...
\begin{figure}[H]
    ...
    still wins \textit{brand} (75.0\,\%) and could be preferred if
...
\begin{figure}[H]
    ...
    3.0\,pp; the Phase-D fine-tune's gap is 2.7\,pp; $r=32$'s gap is
    4.9\,pp; Qwen+$r=32$'s gap is 4.6\,pp. The larger adapter
    overfits devel slightly more, but not enough to lose its
    test-set lead.}
    \label{fig:13-gap}
\end{figure}

Figure~\ref{fig:13-fs-vs-ft-pc} closes the ``few-shot vs fine-tune''
story with a direct per-class comparison. Every class gains; the two
semantically-harder classes gain the most (\textit{drug\_n}~$+37.3$,
\textit{group}~$+32.3$), which is exactly what an in-domain
fine-tune is supposed to deliver.
...
\begin{itemize}[nosep]
    ...
    engineering.} From the best few-shot (39.2\% devel m-F1) to the
    Phase-D fine-tune (59.7\%) is \textbf{+20.5\,pp} absolute. Every
    prompt-or-shot change we tried was worth $\leq$2\,pp; in-domain
    training is an order of magnitude more effective.
    \item \textbf{Rank capacity dominates training time.} $r=32$
    vs~$r=8$ at the same 10~epochs: \textbf{+4.1\,pp~m-F1} and
    \textbf{+8.1\,pp~M-F1} for +12\,min of training time. 15~epochs
    at~$r=8$: only +1.8\,pp for +2\,h~50\,m. The Phase-D adapter was
    ...
    (35.3 vs~37.6\,\%) but \emph{beats} it once fine-tuned
    (61.2~vs~59.7 m-F1, 56.6~vs~51.5 M-F1). Takeaway: do not extrapolate
    ...
    champion (60.8\,\% / 44.7\,\%); on test it loses those titles to
    Llama+$r=32$ (55.7\,\% / 41.7\,\%). \textbf{Llama's
    \textit{drug\_n} F1 actually grew from devel to test
    (+4.4\,pp)}, while Qwen's fell 8.9\,pp. Without Phase~I we would
    have reported the wrong champion.
    \item \textbf{Prompt-only cannot solve rare-class recall.} The
    two best-micro few-shot configurations (p01/15, p03/15) both have
    \textit{drug\_n}~F1~$=$~0. The class-balanced sampler lifts
    \textit{drug\_n} to 25.3\,\% but loses 6\,pp of micro-F1; on
    15~demonstrations you cannot cover head and tail
    simultaneously. Fine-tuning sidesteps this entirely: 37.3\,\% at
    $r=8$, 41.7\,\% on test at~$r=32$ (both beat the best few-shot
    in this category).
    \item \textbf{Devel$\to$test gap is 2.7--4.9\,pp.} None of our
    fine-tune configurations catastrophically overfit, though the
    bigger adapter ($r=32$) does shed 4.9\,pp on the test split vs
    $r=8$'s 2.7\,pp. The Phase-E few-shot transfer gap (3.0\,pp) is
    \emph{smaller} than the $r=32$ fine-tune gap, consistent with
    few-shot having nothing to overfit.
    \item \textbf{\textit{drug\_n} is still the bottleneck.} Our best
    system reaches 41.7\,\% test F1 on \textit{drug\_n}. That is
    roughly $\mathbf{3\times}$ what Systems~1.1 and~1.2 achieve on
    the same rare class (12.6\% and 15.6\,\%). The LLM's rare-class
    handling is the single metric on which System~1.3 dominates. See
...
\begin{figure}[H]
    ...
    System~1.3 system (FT Llama $r=32$) trails System~1.2 by
    $-$28.5\,pp on micro and $-$13.4\,pp on macro. The gap is
    \emph{smaller on macro} --- the LLM is comparatively more competitive on
    the harder classes than on the easy ones.}
...
1.2 BiLSTM & \cellcolor{green!18}\textbf{87.4} & \cellcolor{green!18}\textbf{69.1} & 89.1 & 92.0 & \cellcolor{green!18}\textbf{79.8} & 15.6 \\
1.3 FT-LLM & 58.9 & 55.7 & 68.9 & 61.3 & 51.0 & \cellcolor{green!18}\textbf{41.7} \\
\hline
\end{tabular}
...
$r{=}32$, 10\,ep, 4-bit. CRF/BiLSTM dominate on micro, macro, and
the common classes; the LLM only wins \textit{drug\_n} but does so
by 26--29\,pp.}
\label{tab:conc}
\end{table}
...
\begin{figure}[H]
    ...
    only wins one column (\textit{drug\_n}, the rare class) but does
    so by a very large margin. On the three common classes, the
    CRF's sharp character-level patterns and the BiLSTM's learned
    embeddings dominate by 20--30\,pp.}
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Performance.} On this dataset, the dedicated NERC
    systems win decisively on aggregate metrics (1.2 BiLSTM $=$
    87.4\,\% micro vs 1.3 FT-LLM $=$ 58.9\,\% micro). \textbf{But
    the LLM dominates the rare \textit{drug\_n} class by a factor of
    $\sim$3}, which matters for a drug-interaction-safety downstream
    task where missing an illicit-drug mention is costlier than
    missing a common one. If the downstream metric weights rare
    classes, System~1.3's advantage is real. On macro-F1 (which
    weights all classes equally) the gap narrows from 28\,pp to
    13\,pp.
\end{itemize}

\subsection{Why the LLM under-performs on common classes}

We attribute the 28\,pp micro gap to three factors:

\begin{enumerate}[nosep]
    \item \textbf{Model size.} 3B~parameters at 4-bit is tiny by LLM
    standards. Recent NER-tuned LLMs (e.g.\ \textit{GLiNER},
    \textit{UniNER-7B}) reach DDI-scale performance, but those are
    7--13B~params at full precision and would not fit the cluster.
    ...
    precision in a way that a tagger can never lose points.
...
\begin{itemize}[nosep]
    ...
    (86.8 vs 87.4).
    \item \textbf{Use System~1.2 (BiLSTM)} when best average
    performance matters and a GPU is available. It is the highest
    micro-F1 on this task.
    \item \textbf{Use System~1.3 (LoRA-LLM)} when rare-class recall
    matters, cost and explainability are secondary, and a mid-range
    GPU is available. It buys $\sim$3$\times$ rare-class recall over
    feature-based NERC on this dataset, at $\sim$100$\times$ the
    training cost.
\end{itemize}

\paragraph{Overall conclusion.}
For the DDI~2013 NER task, a dedicated BiLSTM-NN remains the best
overall system (test micro-F1~$\mathbf{87.4\,\%}$). Prompt-engineering
alone, at the 3B-parameter scale our hardware allows, is insufficient
(best 36.2\,\%). LoRA fine-tuning closes the bulk of that gap
(58.9\,\%) and delivers a striking $\mathbf{3\times}$ rare-class
recall advantage (\textit{drug\_n} F1 $41.7$ vs 15.6 vs 12.6), but
still trails dedicated NERC systems on common classes. The single
